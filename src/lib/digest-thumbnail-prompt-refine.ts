...
3. Do **not** instruct the image model to draw legends, sidebar icon keys, blank pastel pills, empty callout boxes, or any “reserved caption” UI—only the biological scene and at most ONE magnified inset or magnifying-glass callout for macro–micro gap when needed. Editorial labels are added as HTML overlays later; the raster must stay legend-free.
4. **Do not list label strings for the image model to paint.** Before your closing lines, restate background choice: for cellular/immune/tissue-mechanism stories, very light warm pink or flesh-toned high-key field with subtle diffuse vascular or ECM context (still airy, not muddy); for devices/pipelines/conceptual scenes, pure clinical white. Always: soft gradients on subjects, muted professional accents, strong readable hierarchy—and horizontal widescreen (16:9 style), not square or portrait.
5. **Final lines (overlay metadata only):** The **last** 2–4 lines of your entire response must each be **only** a short caption phrase editors will reuse as HTML overlay labels (plain words; about 1–6 words per line). **No** section headings, no “OVERLAY_CONCEPTS” prefix, no bullet characters, no numbering, no lines that are only dashes, and no “Caption:” prefixes—each line is **only** the phrase. **Nothing** may follow those lines.
